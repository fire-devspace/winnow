...
public actor TxBroadcaster {
    ...
    private struct Pending {
        ...
        /// [fire-only] Some peer was handed the bytes at least once. Sticky
        /// for the entry's life and persisted: a peer that took the
        /// transaction keeps it, so its later disconnection is not a relay
        /// failure, and a relaunch must not forget that the network has it.
        var served = false
    ...
    private struct StoredPending: Codable {
        ...
        struct StoredTx: Codable {
            ...
            var confirmedAtHeight: UInt32?
            /// [fire-only] Whether some peer was ever served the bytes;
            /// absent in stores written before it, which load as not.
            var served: Bool?
        }
    ...
    /// [fire-only] Whether some peer was ever handed this transaction's
    /// bytes, across disconnections and relaunches. What "seen by the
    /// network" may honestly rest on.
    ...
    private func markPeerFailed(_ peer: PeerConnection, reason: String) {
        ...
        for (txid, entry) in pending {
            // [fire-only] A peer that was handed the bytes keeps them; its
            // going away afterwards is not a failure to relay.
